...
class LinearClassifierByMe(nn.Module):
	def __init__(self, layers, drops):
		super().__init__()
#		print("Layers shape is ",len(layers))
...
#			packed = pack_padded_sequence(embeddings, lengths, batch_first=True)
		print("Layers in PoolingLinearClassifier are ",self.layers)
	...
	def forward(self, input):
		raw_outputs, outputs = input
		#print("raw_outputs shape is ",len(raw_outputs),"outputs shape is ",len(outputs))
		#print("raw_outputs are ",raw_outputs,"outputs are ",outputs)
		output = outputs[-1]
...
#		print("Size of output is ",output.size())
		...
		x=output.transpose(dim0=0, dim1=1) # Batch First
		#print("Type of x is ",x)
...
#		x=x.contiguous().view(-1,400)
		...
		#x=output
		# To here
		#print("x after concat of output",x)
...
#		print("Size of output is ",output.size())
		...
		#x=output.transpose(dim0=0, dim1=1)
		x=output
		# To here
#		print("x after concat of output",x)
...
def get_rnn_classifer(bptt, max_seq, n_class, n_tok, emb_sz, n_hid, n_layers, pad_token, layers, drops, bidir=False,
	...
	rnn_enc = MultiBatchRNN(bptt, max_seq, n_tok, emb_sz, n_hid, n_layers, pad_token=pad_token, bidir=bidir,
					  dropouth=dropouth, dropouti=dropouti, dropoute=dropoute, wdrop=wdrop)
	return SequentialRNN(rnn_enc, LinearClassifierByMe(layers, drops))
